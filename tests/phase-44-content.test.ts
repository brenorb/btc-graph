...
describe("phase 44 musig slice", () => {
  const nodes = loadNodes();
  const byId = new Map(nodes.map((node) => [node.id, node]));

  it("adds the musig node", () => {
    expect(byId.has("protocol.musig")).toBe(true);
  });

  it("anchors musig on schnorr signatures only", () => {
    const node = byId.get("protocol.musig");
  ...
  it("keeps musig focused on aggregated schnorr signing", () => {
    const node = byId.get("protocol.musig");
    expect(node).toBeTruthy();
    if (!node) return;
    ...
    expect(description).toContain("partial signatures");
